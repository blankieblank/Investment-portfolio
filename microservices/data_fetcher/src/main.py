...
async def main():
    ...
    logging.info(f"Async Data Fetcher starting...")
            ...
            while True:
                ...
                for price_data in results:
                    if price_data:
                        ...
                        try:
                            await producer.send_and_wait(KAFKA_TOPIC, value=price_data, key=ticker.encode('utf-8'))
                            logging.info(f"Sent data to Kafka: {price_data}")
                            await redis_client.hset(LATEST_PRICES_KEY, ticker, price)
                            logging.info(f"Cached price in Redis for {ticker}: {price}")
                        except Exception as e:
                            logging.error(f"Error during send/cache for {ticker}: {e}")
